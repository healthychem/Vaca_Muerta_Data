---
title: "Estadística en Vaca Muerta"
author: "Pablo Vidal"
date: "`r Sys.Date()`"
output: 
  html_document:
...
Descubrir que variables correlacionan con la productividad en pozos petroleros horizontales basados en datos históricos y estimar la productividad en un intervalo de tiempo establecido.
...
f_3 <- df_frac_prod_tran %>%
  ...
  facet_wrap(~last_empresa)+
  theme(legend.position = 'none')+
    ggpubr::stat_cor(aes(label = paste(..rr.label..,..r.label.., ..p.label.., sep = "~`,`~")),label.x = 3,label.y = 85000)
...
f_4 <- df_frac_prod_tran %>%
  ...
  facet_wrap(~last_empresa)+
  theme(legend.position = 'none')+
    ggpubr::stat_cor(aes(label = paste(..rr.label..,..r.label.., ..p.label.., sep = "~`,`~")),label.x = 3,label.y = 120000)
